...
export const switchRole = async (req: Request, res: Response) => {
    try {
        ...
        if (!user) return res.status(404).json({ message: "User not found!" }); 
        if (!user.canSwitchRole) return res.status(403).json({ message: "You cannot switch roles" });
        if (!user.isBuyer && !user.isSeller) return res.status(400).json({ message: "No role selected yet, use chooseRole first" });
